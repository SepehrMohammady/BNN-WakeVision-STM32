...
def accuracy_calc(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        # Ensure maxk doesn't exceed the number of classes in the output
...
def main():
    ...
    try:
        model = models.__dict__[args.arch_name](sub_path=arch_cand_tensor, img_size=128 if args.dataset_name.lower() == 'wakevision' else 224)
    except Exception as e:
    ...
    elif args.dataset_name.lower() == 'imagenet':
        dummy_input_shape = (1, 3, 224, 224) 
    elif args.dataset_name.lower() == 'wakevision':
...
# 5. Prepare DataLoader for the TEST/VAL set
    testdir = os.path.join(args.data_dir, 'test')
    if not os.path.exists(testdir):
        testdir = os.path.join(args.data_dir, 'val')
    if not os.path.exists(testdir):
        logger.error(f"Test data directory not found: {args.data_dir}\\test or {args.data_dir}\\val. Please check path.")
        return
        
    normalize = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
    img_size = 128 if args.dataset_name.lower() == 'wakevision' else 224
    if args.dataset_name.lower() == 'wakevision':
        # WakeVision images are pre-resized to 128x128 by prepare_full_wakevision.py.
        # Use the same transform as train_single.py: plain Resize, no CenterCrop.
        val_transform = transforms.Compose([
            transforms.Resize((img_size, img_size)),
            transforms.ToTensor(),
            normalize])
    else:
        # ImageNet-style: resize short edge then center-crop
        val_transform = transforms.Compose([
